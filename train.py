            ...
            if (step % config.log_epoch == 0) and (step != 0):
                ...
                print(
                    "\nStep: [{}/{}]: avg min phase: {:.4f} avg max phase: {:.4f} val accuracy: {:.4f} spd: {:.4f} f1: {:.4f} EOD: {:.4f} ROC: {:.4f} S_accuracy: {:.4f}".format(
                        step, total_step, avg_train_loss, avg_train_loss_max, np.mean(val_accuracy_list)*100, np.mean(spd_list)*100, np.mean(F1_list)*100, np.mean(eod_list)*100, np.mean(roc_auc_list), S_accuracy))
...
def grid_search_train(loader, total_data):
    handler = logging.FileHandler(f'log/train_{config.dataset_name}.log')
    handler.setFormatter(log_format)
    logger.addHandler(handler)
    logger.propagate = False
